...
#'
get_model_grid <- function(template_response, human_psychometrics, 
    level_vec, error_fcn = "mle") {
    library(parallel)
    ecc_dat <- template_response %>% dplyr::select(eccentricity) %>% 
        distinct() %>% mutate(eccentricity)
    ...
    human_pc <- merge(ecc_dat, human_psychometrics) %>% group_by(eccentricity, 
        ...
        arrange(TARGET, BIN, eccentricity) %>% ungroup()
    
    grid_f <- function(x) {
        if (error_fcn == "mle") {
            error <- get_NLL(x, template_response, human_pc)
        } else {
            error <- get_E(x, template_response, human_pc)
        }
        
        upper_limit <- 3000
        grid_search <- expand.grid(seq(0.001, 100, length.out = 10), 
            seq(0.001, 100, length.out = 10), seq(0.001, 100, 
        ...
        b_params <- grid_search %>% mutate(eccentricity = x) %>% 
            filter(!is.na(error)) %>% filter(error == min(error)) %>% 
            head(., 1) %>% print
    }
    ...
    names(full_df) <- c("edge", "lum", "pattern", "bias", "error", 
        "eccentricity")
