...
save_gaussian_distributions <- function(template_response) {
    ...
    mclapply(
      ...
      FUN =
        function(x) {
          ...
          save(
            file = paste0(
              "~/Dropbox/Calen/Work/occluding/detection_model_analysis/_data/_model_response/model_distribution_",
  ...
  save(
    file = paste0(
      "~/Dropbox/Calen/Work/occluding/detection_model_analysis/_data/model_distribution_all.rdata"
    ),
    model.data.all
  )
  save(
