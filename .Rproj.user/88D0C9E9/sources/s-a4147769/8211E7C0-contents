...
plot_psychometric <- function(psychometric.params, empirical.obs, bin = 1, target = "vertical", out_path = "~/Dropbox/Calen/Dropbox/") {
  ...
  empirical.dat <- empirical.obs %>%
    ...
    mutate(human.dat = map(data, function(data) {
      data.frame(dprime = data$dprime, eccentricity = data$eccentricity)
    })) %>%
  ...
  all.dat <- dat %>%
    ...
    mutate(psychometric.dat = map(data, function(data) {
      ...
      psychometric.dat <- data.frame(eccentricity = eccentricity, dprime = dprime)
    })) %>%
    unnest(data) 
    ...
    by_row(all.dat, function(row) {
      ...
      fig <- ggplot(model, aes(x = eccentricity, y = dprime)) + 
        geom_line() +
        geom_point(data = human, inherit.aes = F, aes(x = eccentricity, y = dprime)) +
